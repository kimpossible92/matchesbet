...
public class GamesparksDataManager : IDataManager
{
    ...
    {
        new LeaderboardDataRequest().SetLeaderboardShortCode("Level.Level." + 0).SetDontErrorOnNotSocial(true).SetEntryCount(10).Send((response) =>
        {
            if (!response.HasErrors)
            {
                foreach (GameSparks.Api.Responses.LeaderboardDataResponse._LeaderboardData entry in response.Data)
                {
                    ...
                    TournamentLB.THIS.playerNames.Add(entry.UserName); Debug.Log(entry.UserName);
                    TournamentLB.THIS.ListScores.Add(int.Parse(entry.JSONData["Score"].ToString()));
    ...
    public void GetCallPhotoTour5()
    {
        ...
        bool callme = false;
        int loadint2 = 0;
        string dayStripe = "Stripe1.SNAPSHOT.2019-03-10";
        ...
        new LeaderboardDataRequest().SetLeaderboardShortCode(dayStripe).SetEntryCount(1000).Send((resp1) =>
        {
            if (!resp1.HasErrors)
            {
                foreach (var item1 in resp1.Data)
                {
                    ...
                    //TounamentLB.THIS.playerNames.Add(item1.Rank + ":" + item1.UserName + "$" + item1.UserId);
                    TournamentLB.THIS.playerNames.Add(item1.UserName);Debug.Log(item1.UserName);
                    TournamentLB.THIS.ListScores.Add(int.Parse(item1.JSONData["Stripe"].ToString()));
    ...
    {
        string dayStripe = "Stripe1.SNAPSHOT.2019-03-10";
        string url1 = Tournament.tournament.urlOnTournament.Replace(Tournament.urlpredicate, string.Empty);
        new LogEventRequest()
            .SetEventKey(dayStripe).SetEventAttribute("Stripe", PlayerPrefs.GetInt("TourQual"))
            .Send((resp12) =>
            {
                Debug.Log(resp12.HasErrors);
            });
        
            new LogEventRequest_InputScore()
                .Set_TRACK(url1)
                .Set_score(PlayerPrefs.GetInt("TourQual"))
